...
- **OSP-Next-14B-BF16**：在 Atlas A3 系列（Ascend 910C）上原生 BF16 训练，直接支持昇腾 NPU 推理。
- **OSP-Next-14B-HiF8**：业内首次在昇腾 NPU 上完成 8 比特量化训练和推理的 AIGC 模型。基于 Ascend 950 A5 原生支持的 **HiFloat8（HiF8）数据格式**进行量化感知训练（QAT），并完成了基于 GRPO 方法的 HiF8 强化学习后训练。由于 Atlas A3（Ascend 910C）不原生支持 HiF8，训练阶段借助 `quant_cy_npu` 仿真算子在 BF16 框架下对所有 Linear 投影层及 Attention Q/K/V 输入施加 HiF8 精度仿真，使权重收敛至 HiF8 精度范围。所得权重可在 Ascend 950 A5 上直接进行 HiF8 量化推理，无需格式转换。

本样例的并行策略与性能优化详情可参见 [TODO: 填写技术文档或论文链接]()。

## 硬件要求

- 产品型号：Ascend 950 A5（推理单卡即可，训练至少需要 8 卡服务器）
- 操作系统：Linux ARM
...
从 [TODO: 填写权重下载地址（ModelScope）]() 下载 OSP-Next-14B-BF16 或 OSP-Next-14B-HiF8 完整权重包，上传到服务器固定路径，例如 `/data/models/osp-next-14b-hif8`。
...
OSP-Next-14B 单张 Ascend 950 A5 即可完成推理，无需多卡。
...
- **自定义算子导入失败**：报错含 `"_OpNamespace 'custom' object has no attribute"`，请参考[自定义算子指南](TODO: 填写链接)编译所需算子。
